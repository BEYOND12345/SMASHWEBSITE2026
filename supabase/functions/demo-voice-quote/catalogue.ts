/**
 * Demo catalogue for public voice-to-quote edge function.
 * Keep in sync with src/data/demo-quote-catalogue.ts
 */
...
export function buildDemoQuoteFromTranscript(
  ...
  const lineItems: DemoQuoteLineItem[] = scored.slice(0, 4).map(({ item }) => {
