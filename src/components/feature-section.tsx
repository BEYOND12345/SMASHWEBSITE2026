...
export const FeatureSection: React.FC<FeatureSectionProps> = ({
  ...
  const bgClass = background === 'white' ? 'bg-white' : background === 'slate' ? 'bg-surface' : 'bg-brand text-white';
  const textClass = background === 'brand' ? 'text-white' : 'text-brand';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
        <div className={`${image ? 'grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 lg:gap-20' : 'max-w-3xl'} items-center ${imagePosition === 'left' ? 'lg:flex-row-reverse' : ''}`}>
          <div className={`min-w-0 ${imagePosition === 'left' ? 'lg:order-2' : ''}`}>
            ...
            <div className={`min-w-0 text-sm sm:text-base md:text-lg lg:text-xl font-medium leading-[1.15] mb-5 md:mb-7 ${descClass}`}>
              {description}
            </div>
            ...
            {bullets && bullets.length > 0 && (
              <ul className="space-y-2.5 md:space-y-4">
                {bullets.map((bullet, index) => (
                  <li key={index} className="flex items-start gap-2.5 md:gap-3 min-w-0">
                    ...
                    <span className={`text-xs sm:text-sm md:text-base lg:text-lg font-body font-medium leading-[1.35] min-w-0 ${background === 'brand' ? 'text-white/90' : 'text-brand/80'}`}>
                      {bullet}
                    </span>
                  </li>
                ))}
